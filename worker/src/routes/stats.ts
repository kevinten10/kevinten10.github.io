...
statsRoutes.post('/view', async (c) => {
  ...
  await c.env.DB.prepare('insert into page_stats (page_path, pv, uv, updated_at) values (?, 1, 1, ?) on conflict(page_path) do update set pv = pv + 1, updated_at = ?')
...
statsRoutes.get('/public', async (c) => {
  ...
  const page = await c.env.DB.prepare('select page_path, pv, uv, comments_count, rewards_count from page_stats where page_path = ?').bind(pagePath).first();
  ...
  const data = {
    page: page || { page_path: pagePath, pv: 0, uv: 0, comments_count: 0, rewards_count: 0 },

...
- Vitest: 22 files, 111 tests passed after the stats, payments, QR, DNS, authoritative DNS, Windows `nslookup` fallback, Auth0 CLI automation, cutover audit, preview audit, maintenance audit verification, production HTTP, Access HTTP, and cutover hardening pass.
...
- Preview smoke script passed again on June 28, 2026 against `https://kevinten-interactive-preview.pages.dev` at 2026-06-28T16:11:12Z with latest smoke page `/preview-smoke-1782663041294`; the JSON audit recorded 15 checks, 15 passed, 0 failed, and `homeIncludesAuth0: true`.
...
- `AUTH0_CLI_TIMEOUT_MS=10000 CUTOVER_AUDIT_OUT=docs/maintenance/2026-06-28-cutover-readiness.json node scripts/verify-cutover-readiness.mjs` was rerun at 2026-06-28T16:08:03Z and still reports production not ready with 24 checks: 10 passed and 14 not ready. Auth0 verification can find the public preview runtime client ID, but the local Auth0 CLI refresh token is expired and `--no-input` correctly reports that `auth0 login` is required before automated production callback checks can pass. The WeChat QR is not a collect-money QR, Pages custom domains and Cloudflare DNS records cannot be read with the active credential, Cloudflare zone status is `initializing`, `.com` registry delegation is correct, Cloudflare authoritative nameservers return `NOERROR` with zero answers for both production hosts, recursive DNS currently returns no usable production answers, and `https://kevinten.com/` plus `https://www.kevinten.com/` fail TLS handshakes instead of serving Cloudflare Pages and Cloudflare Access.

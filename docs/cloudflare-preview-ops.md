...
`npm run verify:cutover` checks Worker CORS, Pages runtime, Auth0 callbacks, reward QR codes, Pages custom domains, active Cloudflare zone status, Cloudflare DNS records for the apex and `www` CNAMEs, registry delegation, direct Cloudflare authoritative DNS answers from `chip.ns.cloudflare.com` and `faye.ns.cloudflare.com`, recursive DNS, production HTTP responses for the apex and `www` domains, unauthenticated production `/admin/` HTTP protection, and Access API protection for production admin paths. Cloudflare API read failures are reported as `not ready` without intentionally stopping later independent checks. The authoritative DNS gate expects `NOERROR` with a non-GitHub answer; `REFUSED` means Cloudflare has not started serving the zone yet. The production HTTP gate expects Cloudflare response evidence and the generated runtime marker; a GitHub Pages response is still treated as not ready even if it returns HTTP 200. The admin HTTP gate expects a Cloudflare Access login/challenge marker and Cloudflare response evidence; a GitHub 404/admin shell is not sufficient. If Node `fetch` fails, the script falls back to `curl` and parses final response headers after redirects before evaluating `server`, `cf-ray`, and GitHub headers. Set `AUTH0_CLI_TIMEOUT_MS` when diagnosing a slow or stuck Auth0 CLI; the script reports Auth0 as `not ready` after the timeout instead of hanging. Set `CUTOVER_AUDIT_OUT=docs/maintenance/2026-06-28-cutover-readiness.json` to save the same checks as structured JSON with pass/fail counts.

As of June 28, 2026, the check is expected to remain `not ready` until `AUTH0_CLIENT_ID` is supplied locally and Auth0 CLI access is refreshed, the WeChat QR image is replaced with a real collect-money QR, Cloudflare activates the zone and Pages custom domains, Cloudflare authoritative nameservers stop returning `REFUSED`, the active Cloudflare credential can read zone DNS records, recursive DNS stops returning GitHub Pages/HiChina, production HTTP for `kevinten.com` and `www.kevinten.com` is served by Cloudflare Pages, production `/admin/` shows Cloudflare Access protection, and the active Cloudflare credential can read Access apps.
